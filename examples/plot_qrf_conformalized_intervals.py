...
def plot_prediction_intervals_by_strategy(df):
    ...
    # Single shared slider for varying the target coverage level.
    ...
    def _plot_prediction_intervals(df, domain, suffix=""):
        click = alt.selection_point(name=f"click{suffix}", bind="legend", fields=["y_label"])
    ...
    for i, strategy in enumerate(strategies.keys()):
        ...
        panel = _plot_prediction_intervals(df_i, domain, suffix=f"_{i}")
        panels.append(panel.properties(height=225, width=300, title=strategies[strategy]))

    chart = alt.hconcat(*panels).add_params(coverage_val)
